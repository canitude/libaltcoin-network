...
static int start_result(p2p<message_subscriber>& network)
{
...
static int connect_result(p2p<message_subscriber>& network, const config::endpoint& host)
{
    std::promise<code> promise;
    const auto handler = [&promise](code ec, channel<message_subscriber>::ptr)
    {
        promise.set_value(ec);
    };
...
static int run_result(p2p<message_subscriber>& network)
{
    std::promise<code> promise;
    const auto handler = [&promise](code ec)
    {
...
static int subscribe_result(p2p<message_subscriber>& network)
{
    std::promise<code> promise;
    const auto handler = [&promise](code ec, channel<message_subscriber>::ptr)
    {
...
static int subscribe_connect1_result(p2p<message_subscriber>& network, const config::endpoint& host)
{
    std::promise<code> promise;
    const auto handler = [&promise](code ec, channel<message_subscriber>::ptr)
    {
...
static int subscribe_connect2_result(p2p<message_subscriber>& network, const config::endpoint& host)
{
    std::promise<code> promise;
    const auto handler = [&promise](code ec, channel<message_subscriber>::ptr)
    {
...
static int send_result(const Message& message, p2p<message_subscriber>& network, int channels)
{
    const auto channel_counter = [&channels](code ec, channel<message_subscriber>::ptr channel)
    {
...
BOOST_AUTO_TEST_CASE(p2p__top_block__default__zero_null_hash)
{
    ...
    p2p<message_subscriber> network(configuration);
...
BOOST_AUTO_TEST_CASE(p2p__subscribe__started_stop__service_stopped)
{
    ...
    std::promise<code> promise;
    const auto handler = [](code ec, channel<message_subscriber>::ptr channel)
    {
...
////    const auto subscribe_handler = [&subscribe, &network](code ec, channel<message_subscriber>::ptr)
////    {
